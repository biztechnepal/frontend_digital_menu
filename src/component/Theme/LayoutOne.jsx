...
function LayoutOne({
    ...
    return (<>
        <Page title="Company Menu" sx={{ backgroundColor: "#fdfdfd", fontFamily: theme?.font }}>
            ...
            <FormHeader onHandleChange={handleFilter} filterName={menuName} containerStyle={{ backgroundColor: 'white', borderRadius: '5%', width: '50%', margin: '0 auto' }} />
            ...
            {
                isPopup && popup?.length > 0 &&
                popup.map((item, i) => <PopupAdvertise key={i} url={`${import.meta.env.VITE_APP_HOST_API_KEY}/${ENDPOINTS.DOWNLOADPOPUPSFILE}/${item.id}`} open={isPopup} onClose={() => setIsPopup(false)} />)
            }
        </Page>
    </>
    )
